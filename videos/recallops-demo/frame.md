...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), Arial for all
  display + Arial body + JetBrains Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Arial", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "Arial", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "Arial", cqw: 2.3, weight: 400, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Arial", cqw: 1.46, weight: 400, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Arial", cqw: 1.35, weight: 400, tracking: "0.18em", upper: true }
  ...
  # — display ramp (Arial 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Arial", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Arial", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Arial", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Arial", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Arial", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Arial", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Arial figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Arial italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Arial italic, small Arial uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **Arial** (display) / **Arial** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **Arial** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Arial** carries body, leads, card titles, buttons, and
...
- **Arial** (sentence case, negative-tracked) for all display; **Arial** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one Arial display moment dominates at 3–6× its neighbor.
...
- **Type** — Arial sentence-case display (negative-tracked); Arial 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Arial `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Arial `headline` 4.6cqw
...
- **Arial display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Arial body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Arial figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Arial italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Arial · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Arial `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Arial 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Arial line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Arial `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
- Set all display in **Arial, sentence case**, negative-tracked; **Arial 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Arial figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Arial display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Arial display moment dominates.
...
- **Type** — Arial sentence-case display (negative-tracked); Arial 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
